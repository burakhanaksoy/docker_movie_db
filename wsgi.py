...
def add_movie():
    ...
    mov_title = data['mov_title']
    mov_time = data['mov_time']
    if mov_time and mov_title:
        new_movie = Movies(mov_title, mov_time)
        db.session.add(new_movie)
        db.session.commit()
        return f'{mov_title} created'
    else:
        return f'Null value detected!'
